...
> [!IMPORTANT]
> **AIエージェントの動作について**:
> 本プロジェクトに参加するAIエージェントは、`.agent/workflows/standard_flow.md` に定義されたチェックリストを各セッションの開始時に必ず読み込み、そのプロセス（Issue作成、設計判断の記録等）を厳守してください。
...
### ライフサイクル
0. **Issue作成 (Dual Mandatory)**: 作業開始前に必ず **GitHub 上で Issue を作成** し、同時に **`docs/issues/` 下にローカルドキュメントを作成** する。この二重管理は AI エージェントのコンテキスト共有における必須要件とする。
1. **ブランチ作成**: `main` からブランチを切る。命名規則に従う。
2. **コミット**: コミットメッセージにはIssue番号を含める（例: `feat: #8 ログイン画面を実装`）。日本語推奨。
3. **マージ**: 実装と検証が完了し、問題がないと判断できれば `main` へマージし、ブランチを削除する。
    - **【Alpha版附則】**: α版の間は、ブランチ作成および実装完了後、即座に `main` へマージしてデプロイを優先する運用を許可します。
6. **完了報告**: 最終報告には GitHub Issue とローカル Issue ファイルの両方のリンクを提示するとともに、**本番環境のデプロイURL (https://74min.vercel.app)** を必ず含める。
